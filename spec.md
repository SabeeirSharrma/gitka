...
```
/ (root of USB)
├── gitc(.exe)                    # the one binary — everything runs through this
...
Triggered by `gitc unlock <repo>` (local-only) or `gitc serve <repo>` (GitFlare LAN):

```
gitc unlock coalbox          OR       gitc serve coalbox
...
gitc lock coalbox             OR       gitc serve --stop
...
Runs per repo, every `gitc sync` (against whichever repos are currently extracted, or against archived repos by extracting → comparing → recompressing transiently):
...
```
gitc                    # no subcommand → launches GUI
gitc init                 # wizard: source auth, selection, target, compression, integrations
gitc scan                   # re-scan sources + target, show size report + budget check
gitc sync                     # fetch/compare/push/pull/merge loop across selected repos
gitc status                     # per-repo state: ahead/behind/conflict, last synced
gitc unlock <repo>                # extract for local-only offline commit access
gitc lock <repo>                    # recompress + clear extraction, end local-only session
gitc serve <repo>                     # extract + start GitFlare LAN server
gitc serve --stop                       # stop LAN server, recompress, clear extraction
gitc verify                               # manual integrity + permissions check
gitc repair <repo>                          # use recovery record to fix a corrupted repo
gitc config                                   # view/edit TOML config
gitc gui                                        # explicitly launch GUI (same as bare `gitc`)
```
