---
title: CLI Command Reference
description: Gitka CLI commands
---

# CLI Command Reference

## Setup & Configuration

| Command | Description |
|---|---|
| `gitka wipe` | Wipe removable drive + set up (SAFETY: removable only, requires YES) |
| `gitka init` | Initialize a new Gitka backup on existing storage |
| `gitka init --volume-size <MB>` | Initialize with volume splitting |
| `gitka init --dedup` | Initialize with cross-repo deduplication |
| `gitka config` | View/edit configuration |
| `gitka config --get <key>` | Get a config value |
| `gitka config --set <key>=<value>` | Set a config value |

## Repository Management

| Command | Description |
|---|---|
| `gitka scan` | Discover repos from source, show budget |
| `gitka sync` | Clone/fetch repos (incremental — skips untouched repos) |
| `gitka sync --json` | Sync with JSON output for scripting |
| `gitka status` | Per-repo state, archive sizes, active sessions |
| `gitka status --json` | Status with JSON output |

## Offline Workflow

| Command | Description |
|---|---|
| `gitka unlock <repo>` | Extract for offline commit access |
| `gitka lock <repo>` | Recompress + close session (shows audit trail) |

## LAN Sharing

| Command | Description |
|---|---|
| `gitka serve <repo>` | Serve over LAN via GitFlare |
| `gitka serve <repo> --stop` | Stop the LAN server |

## Integrity & Repair

| Command | Description |
|---|---|
| `gitka verify` | Check archive integrity |
| `gitka repair <repo>` | Fix corrupted repo with recovery records |

## Authentication

| Command | Description |
|---|---|
| `gitka auth login` | Authenticate with GitHub/GitFlare personal access token |
| `gitka auth status` | Show current authentication status |
| `gitka auth logout` | Remove stored credentials |

## Utilities

| Command | Description |
|---|---|
| `gitka update` | Update CLI and GUI to the latest version (rebuilds from source) |
| `gitka update --check` | Check for updates without installing |
| `gitka update --no-gui` | Update CLI only, skip GUI |
| `gitka update --json` | JSON output for scripting |
| `gitka train-dict <repo>` | Train zstd dictionary on a specific repo for better compression |
